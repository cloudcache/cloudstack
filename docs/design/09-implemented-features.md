...
2. [固定 IP 容器网络（Multus + bridge）](#2-固定-ip-容器网络multus--bridge)
...
使用 **Multus CNI** 为每个 Pod 附加一个（或两个）辅助网卡，通过 **Linux bridge** 将 Pod 接入宿主机所在的 L2 网络，从而获得宿主机网段内的稳定 IP。
...
应用部署时，QuickStack 从两个池各分配一个 IP，Pod 获得两个 bridge 辅助网卡（`net1`=vpc，`net2`=pub）。
...
- Linux bridge 模式下宿主机可以直接与 Pod 通信（与 macvlan 不同），简化了平台基础设施对 Pod 的访问。
