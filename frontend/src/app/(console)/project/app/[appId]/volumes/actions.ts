...
import { volumeBackupEditZodModel } from "@/shared/model/backup-volume-edit.model";
...
const appVolumeActionSchema = appVolumeEditZodModel.merge(z.object({
    appId: z.string().min(1),
    id: z.string().nullish(),
}));

export const saveVolume = async (prevState: any, inputData: z.infer<typeof appVolumeActionSchema>) =>
    saveFormAction(inputData, appVolumeActionSchema, async (validatedData) => {
        await isAuthorizedWriteForApp(validatedData.appId);
        const token = await getBackendToken();
        const app = await backend.apps.getById(token, validatedData.appId);
        const payload = {
            name: validatedData.containerMountPath,
            container_mount_path: validatedData.containerMountPath,
            share_with_others: validatedData.shareWithOtherApps,
            shared_volume_id: validatedData.sharedVolumeId ?? undefined,
        };

        if (validatedData.id) {
            await backend.apps.managedVolumes.update(token, app.project_id, validatedData.appId, validatedData.id, payload);
        } else {
            await backend.apps.managedVolumes.create(token, app.project_id, validatedData.appId, payload);
        }
        return new SuccessActionResult();
    });
...
const backupActionSchema = volumeBackupEditZodModel.merge(z.object({
    appId: z.string().min(1),
}));

export const saveBackupVolume = async (prevState: any, inputData: z.infer<typeof backupActionSchema>) =>
    saveFormAction(inputData, backupActionSchema, async (validatedData) => {
        await isAuthorizedWriteForApp(validatedData.appId);
        const token = await getBackendToken();
        const app = await backend.apps.getById(token, validatedData.appId);
        ...
        if (validatedData.id) {
            await backend.apps.managedVolumes.backups.update(
                token, app.project_id, validatedData.appId, validatedData.volumeId, validatedData.id,
                {
                    cron_expr: validatedData.cron,
                    retention_days: validatedData.retention,
                }
            );
        } else {
            await backend.apps.managedVolumes.backups.create(
                token, app.project_id, validatedData.appId, validatedData.volumeId,
                {
                    s3_target_id: validatedData.targetId,
                    cron_expr: validatedData.cron,
                    retention_days: validatedData.retention,
                    use_db_backup: validatedData.useDatabaseBackup,
                }
            );
        }
        return new SuccessActionResult();
