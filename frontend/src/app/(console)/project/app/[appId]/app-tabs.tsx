...
export default function AppTabs({
    ...
    return (
        <Tabs defaultValue="general" value={tabName} onValueChange={(newTab) => openTab(newTab)} className="space-y-4">
            <ScrollArea>
                <TabsList>
                    <TabsTrigger value="overview"><Eye className="mr-2 h-4 w-4" />{t("page.app.overview")}</TabsTrigger>
                    {app.app_type !== 'APP' && <TabsTrigger value="credentials"><Key className="mr-2 h-4 w-4" />{t("page.app.credentials")}</TabsTrigger>}
                    <TabsTrigger value="general"><Settings className="mr-2 h-4 w-4" />{t("page.app.general")}</TabsTrigger>
                    <TabsTrigger value="environment"><Zap className="mr-2 h-4 w-4" />{t("page.app.environment")}</TabsTrigger>
                    <TabsTrigger value="domains"><Globe className="mr-2 h-4 w-4" />{t("page.app.domains")}</TabsTrigger>
                    <TabsTrigger value="storage"><HardDrive className="mr-2 h-4 w-4" />{t("page.app.storage")}</TabsTrigger>
                    <TabsTrigger value="advanced"><Cog className="mr-2 h-4 w-4" />{t("page.app.advanced")}</TabsTrigger>
                </TabsList>
