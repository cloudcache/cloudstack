...
export default function RoleEditOverlay({ children, userGroup, projects }: {
  ...
  const form = useForm<RoleEditModel>({
    resolver: zodResolver(roleEditZodModel) as any,
  ...
  useEffect(() => {
    ...
    FormUtils.mapValidationErrorsToForm<typeof roleEditZodModel>(state, form as any);
  }, [state]);
    ...
    <>
      ...
      <Dialog open={!!isOpen} onOpenChange={(isOpened) => setIsOpen(isOpened)}>
        <DialogContent className="sm:max-w-[900px]">
          <DialogHeader>
            <DialogTitle>{userGroup?.id ? t('settings.groups.editGroup') : t('settings.groups.createGroup')}</DialogTitle>
          </DialogHeader>
              ...
              <Form {...form}>
                <form action={(e) => form.handleSubmit((data) => {
                  ...
                  <div className="space-y-4">
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                          ...
                          <div className="space-y-1 leading-none">
                            <FormLabel>
                              {t('settings.groups.canAccessBackups')}
                            </FormLabel>
                            <FormDescription>
                              {t('settings.groups.canAccessBackupsDescription')}
                            </FormDescription>
                          </div>
                        </FormItem>
                      )}
                    ...
                    <div className="pt-3">
                      <h3 className="text-sm font-medium mb-2">{t('settings.groups.appPermissions')}</h3>
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead>{t('plans.projects')}</TableHead>
                            <TableHead>{t('settings.groups.individualPermissions')}</TableHead>
                            <TableHead>{t('settings.groups.readApps')}</TableHead>
                            <TableHead>{t('settings.groups.editDeployApps')}</TableHead>
                            <TableHead>{t('settings.groups.createApps')}</TableHead>
                            <TableHead>{t('settings.groups.deleteApps')}</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                              ...
                              <>
                                <TableRow key={project.id} className={(permission?.roleAppPermissions.length ?? 0) === 0 ? 'border-b-gray-400' : ''} >
                                  ...
                                  {permission?.setPermissionsPerApp ?
                                    <TableHead>{t('settings.groups.app')}</TableHead>
                                    : <TableCell>
                                      <Checkbox
                                        id={`read-${project.id}`}
                                  ...
                                  {permission?.setPermissionsPerApp ?
                                    <TableHead>{t('settings.groups.read')}</TableHead>
                                    : <TableCell>
                                      <Checkbox
                                  ...
                                  {permission?.setPermissionsPerApp ?
                                    <TableHead>{t('settings.groups.readWriteDeploy')}</TableHead>
                                    : <TableCell>
                                      <Checkbox
                                        id={`delete-${project.id}`}
