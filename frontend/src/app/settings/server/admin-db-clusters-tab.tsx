...
export default function AdminDbClustersTab({ initialItems }: { initialItems: DbCluster[] }) {
    ...
    const handleDelete = async (item: DbCluster) => {
        const confirmed = await openConfirmDialog({
            title: t('admin.dbClusters.deleteTitle'),
            description: t('admin.dbClusters.deleteDescription', { name: item.name }),
        ...
        <>
            <Card>
                <CardHeader className="flex flex-row items-start justify-between">
                    <div>
                        <CardTitle>{t('admin.dbClusters.title')}</CardTitle>
                        <CardDescription>{t('admin.dbClusters.description')}</CardDescription>
                    </div>
                    <Button size="sm" onClick={() => setShowCreate(true)}><Plus className="mr-2 h-4 w-4" />{t('common.add')}</Button>
                </CardHeader>
                <CardContent>
                    {items.length === 0 ? (
                        <p className="text-muted-foreground text-sm">{t('admin.dbClusters.empty')}</p>
                    ) : (
                            ...
                            {items.map((item) => (
                                <div key={item.id} className="flex items-center justify-between border rounded-lg p-3">
                                    <div className="space-y-0.5">
                                        ...
                                        <div className="text-xs text-muted-foreground">{t('common.user')}: {item.admin_user}</div>
                                        {item.max_databases !== undefined && (
                                            <div className="text-xs text-muted-foreground">{t('admin.dbClusters.maxDbs')}: {item.max_databases}</div>
                                        )}
            ...
            <Dialog open={showCreate} onOpenChange={setShowCreate}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>{t('admin.dbClusters.addTitle')}</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <Label>{t('admin.dbClusters.dbType')}</Label>
