...
export default function AdminProxyManagersTab({ initialItems }: { initialItems: ProxyManager[] }) {
    ...
    const handleDelete = async (item: ProxyManager) => {
        const confirmed = await openConfirmDialog({
            title: t('admin.proxyManagers.deleteTitle'),
            description: t('admin.proxyManagers.deleteDescription', { name: item.name }),
        ...
        <>
            <Card>
                <CardHeader className="flex flex-row items-start justify-between">
                    <div>
                        <CardTitle>{t('admin.proxyManagers.title')}</CardTitle>
                        <CardDescription>{t('admin.proxyManagers.description')}</CardDescription>
                    </div>
                    <Button size="sm" onClick={() => setShowCreate(true)}><Plus className="mr-2 h-4 w-4" />{t('common.add')}</Button>
                </CardHeader>
                <CardContent>
                    {items.length === 0 ? (
                        <p className="text-muted-foreground text-sm">{t('admin.proxyManagers.empty')}</p>
                    ) : (
            ...
            <Dialog open={showCreate} onOpenChange={setShowCreate}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{t('admin.proxyManagers.addTitle')}</DialogTitle>
                    </DialogHeader>
