...
// ─── DB tools (dbgate / phpmyadmin / pgadmin) ────────────────────────────────

/// GET /api/v1/projects/:pid/apps/:aid/db-tools
pub async fn list_db_tools(
    State(state): State<AppState>,
    Extension(auth): Extension<AuthUser>,
    Path((project_id, app_id)): Path<(String, String)>,
) -> AppResult<impl IntoResponse> {
    super::projects::check_project_access(&state, &auth, &project_id, "OBSERVER").await?;
    let rows = sqlx::query!(
        r#"SELECT id, tool, status, access_url, username, created_at
           FROM app_db_tools WHERE app_id = ? ORDER BY created_at"#,
        app_id
    )
    .fetch_all(&state.db)
    .await?;
...
#[derive(Deserialize)]
pub struct DeployDbToolRequest {
    pub tool: String,
}

/// POST /api/v1/projects/:pid/apps/:aid/db-tools
/// Stub: creates a DB tool record — actual K8s deployment TBD.
pub async fn deploy_db_tool(
    State(state): State<AppState>,
    Extension(auth): Extension<AuthUser>,
    Path((project_id, app_id)): Path<(String, String)>,
    Json(body): Json<DeployDbToolRequest>,
) -> AppResult<impl IntoResponse> {
    super::projects::check_project_access(&state, &auth, &project_id, "OPERATOR").await?;
    let id = Uuid::new_v4().to_string();
    let username = "admin";
    let password = Uuid::new_v4().to_string();
    // TODO: deploy the tool container via K8s when K8s integration is ready
    sqlx::query!(
        r#"INSERT INTO app_db_tools (id, app_id, tool, status, username, password)
           VALUES (?, ?, ?, 'STARTING', ?, ?)
           ON DUPLICATE KEY UPDATE status = 'STARTING', username = VALUES(username), password = VALUES(password)"#,
        id, app_id, body.tool, username, password,
    )
    .execute(&state.db)
    .await?;
...
pub async fn get_db_tool(
    State(state): State<AppState>,
    Extension(auth): Extension<AuthUser>,
    Path((project_id, app_id, tool)): Path<(String, String, String)>,
) -> AppResult<impl IntoResponse> {
    super::projects::check_project_access(&state, &auth, &project_id, "OBSERVER").await?;
    let row = sqlx::query!(
    ...
    .fetch_optional(&state.db)
    .await?
    .ok_or_else(|| AppError::NotFound(format!("db tool {tool}")))?;
...
/// DELETE /api/v1/projects/:pid/apps/:aid/db-tools/:tool
pub async fn delete_db_tool(
    State(state): State<AppState>,
    Extension(auth): Extension<AuthUser>,
    Path((project_id, app_id, tool)): Path<(String, String, String)>,
) -> AppResult<impl IntoResponse> {
