...
pub async fn deploy_from_template(
    ...
    // ── Build env-var list (pure; no DB) before opening the transaction ────
    // Parse template's envVars block + apply input overrides + binding vars.
    ...
    // ── Persist app + sub-resources atomically ─────────────────────────────
    // All inserts share one transaction so a failure can't leave an orphan app
    // row (or half-written ports/env/bindings). External databases provisioned
    // earlier in this deploy live outside SQL, so on rollback they are dropped
    // best-effort below.
    ...
    if let Err(e) = write_result {
        ...
        // The app was not created — drop any external databases provisioned
        // earlier in this deploy so they don't orphan (and keep counting
        // against the project's DB-instance quota). Best-effort.
        for rec in &binding_records {
            if rec.provisioned && rec.kind == "database_instance" {
                if let Err(ce) = drop_provisioned_database(&state, &rec.ref_id).await {
                    tracing::warn!(
                        instance = %rec.ref_id,
                        "from-template rollback: failed to drop provisioned database: {ce}"
                    );
                }
            }
        }
