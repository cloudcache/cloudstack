...
pub async fn deploy_app(
    ...
    let app = sqlx::query!(
        r#"SELECT a.name, a.display_name, a.replicas,
                  ...
                  a.anti_affinity_enabled, a.network_policy, a.source_type,
    ...
    for e in env_vars {
        let val = if e.is_secret != 0 {
            e.value.as_deref().map(|v| state.crypto.decrypt(v)).transpose()?.unwrap_or_default()
        } else {
    ...
    let registry = sqlx::query_scalar!(
        r#"SELECT `value` FROM platform_config WHERE `key` = 'registry_host'"#
    )
    .fetch_optional(&state.db)
    .await?
    .unwrap_or_default();
    ...
    let container_command: Option<Vec<String>> = app
        .container_command.as_deref().map(|s| vec![s.to_string()]);
    let container_args: Option<Vec<String>> = app
        .container_args.and_then(|v| serde_json::from_value(v).ok());
    ...
    let cfg = AppPodConfig {
        ...
        health_check_scheme:  app.health_check_scheme.clone().unwrap_or_else(|| "HTTP".into()),
    ...
    deploy_api
        .patch(&app.name, &PatchParams::apply("quickstack"), &Patch::Apply(&deployment))
        .await
        .or(deploy_api.create(&PostParams::default(), &deployment).await.map(|_| deployment.clone()))?;

    // NodePort Service
    ensure_app_service(state, &client, &ns, app_id, &app.name, &labels).await?;

    // NetworkPolicy (only when not ALLOW_ALL)
    ensure_network_policy(&client, &ns, &app.name, &app.network_policy).await?;

    // Mark DEPLOYING; background status_sync will transition to RUNNING/FAILED
    sqlx::query!(r#"UPDATE apps SET status = 'DEPLOYING' WHERE id = ?"#, app_id)
        .execute(&state.db)
        .await?;
...
async fn ensure_registry_secret(
    ...
    let auth_b64 = base64::engine::general_purpose::STANDARD.encode(format!("{username}:{password}"));
    ...
    let _ = api.patch(&secret_name, &PatchParams::apply("quickstack"), &Patch::Apply(&secret))
        .await;
...
async fn ensure_network_policy(
    ...
    let (ingress, egress, policy_types) = match policy {
        "NAMESPACE_ONLY" => {
        ...
        "DENY_ALL" => (
            Some(vec![]),  // empty slice = deny all ingress
            Some(vec![]),  // empty slice = deny all egress
            vec!["Ingress".to_string(), "Egress".to_string()],
        ),
        "INTERNET_ONLY" => (
            Some(vec![]),  // deny all ingress from cluster
            None,          // allow all egress (app can reach internet)
            vec!["Ingress".to_string()],
        ),
        _ => return Ok(()),
    };
...
async fn ensure_app_service(
    ...
    for port in &ports {
        let nodeport = if let Some(np) = port.nodeport {
            ...
            sqlx::query!(r#"UPDATE app_ports SET nodeport = ? WHERE id = ?"#, np, port.id)
                .execute(&state.db)
                .await?;
            np
        };
    ...
    svc_api.patch(app_name, &PatchParams::apply("quickstack"), &Patch::Apply(&service)).await?;
...
pub async fn delete_app_resources(
    ...
    let _ = np_api.delete(&format!("qs-{app_name}"), &DeleteParams::default()).await;
    let _ = cm_api.delete(&format!("qs-files-{}", &app_id[..8.min(app_id.len())]), &DeleteParams::default()).await;
...
pub async fn scale_deployment(
    ...
    deploy_api.patch(app_name, &PatchParams::default(), &Patch::Merge(patch)).await?;
    Ok(())
}

/// NodePort allocation: find lowest unused port in 30000–32767 (skip 30100 for Registry).
pub async fn allocate_nodeport(state: &AppState) -> AppResult<u16> {
    let used: std::collections::HashSet<u16> = sqlx::query_scalar!(
        r#"SELECT nodeport FROM app_ports WHERE nodeport IS NOT NULL"#
    )
    .fetch_all(&state.db)
    .await?
    .into_iter()
    ...
    let reserved: std::collections::HashSet<u16> = [30100u16].into();
...
pub async fn log_stream(
    ...
    ns: &str,
    app_name: &str,
) -> AppResult<impl futures::Stream<Item = Result<axum::response::sse::Event, std::convert::Infallible>> + Send + 'static> {
    ...
    let pod_name = pods
        .items.into_iter().next().and_then(|p| p.metadata.name)
    ...
    let log_reader = pod_api
        .log_stream(&pod_name, &LogParams { follow: true, tail_lines: Some(100), ..Default::default() })
        .await?;
...
pub async fn handle_terminal(
    ...
    let client = match super::client_for_cluster(&state, &cluster_id).await {
        Ok(c) => c,
        Err(e) => { tracing::error!("terminal: k8s client error: {e}"); return; }
    };

    let pod_api: Api<Pod> = Api::namespaced(client, &ns);
    let pods = match pod_api.list(&ListParams::default().labels(&format!("qs-app={app_name}"))).await {
        Ok(p) => p,
        Err(e) => { tracing::error!("terminal: list pods error: {e}"); return; }
    };

    let pod_name = match pods.items.into_iter().next().and_then(|p| p.metadata.name) {
        Some(n) => n,
        None => { tracing::warn!("terminal: no pods for {app_name}"); return; }
    };

    let mut attached = match pod_api.exec(
        &pod_name, vec!["sh"],
        &kube::api::AttachParams { stdin: true, stdout: true, stderr: true, tty: true, ..Default::default() },
    ).await {
        Ok(a) => a,
        Err(e) => { tracing::error!("terminal: exec error: {e}"); return; }
    };
