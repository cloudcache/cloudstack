...
export const apps = {
    ...
    logsUrl(project_id: string, app_id: string, token?: string): string {
        const base = `${process.env.NEXT_PUBLIC_BACKEND_URL ?? ''}/api/v1/projects/${project_id}/apps/${app_id}/logs`;
    ...
    buildLogsUrl(project_id: string, app_id: string, build_id: string, token?: string): string {
        const base = `${process.env.NEXT_PUBLIC_BACKEND_URL ?? ''}/api/v1/projects/${project_id}/apps/${app_id}/builds/${build_id}/logs`;
        return token ? `${base}?token=${encodeURIComponent(token)}` : base;
    },
    ...
    /** GET /api/v1/projects/:project_id/apps/:app_id/builds/:build_id/logs  — SSE URL */
    buildLogsUrl(token: string, project_id: string, app_id: string, build_id: string): string {
        return `${V1}/projects/${project_id}/apps/${app_id}/builds/${build_id}/logs?token=${encodeURIComponent(token)}`;
    },
...
export const adminUsers = {
    ...
    /** PUT /api/v1/admin/users/:id  { display_name?, email?, is_active?, is_global_admin? } */
    update(token: string, id: string, body: { display_name?: string; email?: string; is_active?: boolean; is_global_admin?: boolean }): Promise<void> {
...
export const adminClusters = {
     ...
     */
    create(token: string, body: { pool_id: string; name: string; display_name?: string; description?: string; k3s_token?: string; vpc_pool_id?: string; pub_pool_id?: string; node_main_iface?: string }): Promise<{ id: string }> {
    ...
    /** PUT /api/v1/admin/clusters/:id  { display_name?, description?, is_active?, vpc_pool_id?, pub_pool_id?, node_main_iface? } */
    update(token: string, id: string, body: { display_name?: string; description?: string; is_active?: boolean; vpc_pool_id?: string; pub_pool_id?: string; node_main_iface?: string }): Promise<void> {
